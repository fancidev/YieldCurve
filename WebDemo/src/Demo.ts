...
namespace Demo {
    ...
    export function drawYieldCurve(
        ...
        const options: YieldCurveFittingOptions = {};
        const discount = fitYieldCurve(model, usedInstruments, usedMarketRates, options);
		
        // Display model info.
        $('#debugOutput').text('Fitted in ' + options.numIter + ' iterations.');
        if (model.info) {
